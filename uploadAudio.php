...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$file || $file['error'] === UPLOAD_ERR_NO_FILE) {
        ...
        if (!in_array($ext, $audioExts)) {
            $error = 'Only audio files are allowed (' . implode(', ', $audioExts) . ').';
        } else {
            $name = preg_replace('/[^a-zA-Z0-9_\-.]/', '_', basename($file['name']));
            $dest = __DIR__ . '/uploads/' . $name;
            if (file_exists($dest)) {
                $error = 'A file named "' . htmlspecialchars($name) . '" already exists.';
  ...
  <body>
    <div class="w3-container w3-padding-24">
      ...
      <div class="w3-card w3-padding w3-green" style="max-width:560px">
        <h2 style="margin-top:0">Upload audio</h2>
        <?php if ($error): ?>
        ...
        <form method="post" enctype="multipart/form-data">
          ...
          <button type="submit" class="w3-button w3-white w3-text-green w3-round">Upload</button>
        </form>
      </div>
